...
#                     manifest = biplist.readPlist(infile)
    ...
    def getBackupFilesList(self):
        ...
        backupFiles = catalog.cursor().execute("SELECT * FROM Files ORDER BY domain,relativePath").fetchall()
...
#             info['backupFile']=f['fileID']
    ...
    def getDomains(self):
        ...
        domains=catalog.cursor().execute("SELECT DISTINCT domain FROM Files").fetchall()
    ...
    def getFileManifestDBEntry(self, fileNameHash=None, relativePath=None):
        ...
        if fileNameHash==None and relativePath==None:
            raise Exception("Either fileNameHash or relativePath must be provided")
